...
| **v23** | the **transfer suite** — broad uplift across lists/strings/numbers (4 arms incl. oracle ceiling) | **1.7B+airfoil vs ALONE: lists 10→27%, strings 7→57%, numbers 7→31%** (+17/+50/+25, all 3 domains; 360M net-negative). But +AIRFOIL ≈ **blind** (vs-blind ≈0); captures 42-73% of oracle | **two findings:** (1) the scaffold is a **real, broad, size-scaling 3-8× amplifier** of a small LLM vs its single-pass self; (2) the uplift is **search-driven, not recognition-driven** — naming doesn't beat blind yet. Next lever = close the combo→oracle gap → **v24** (better recognition) |
| **v24** | close the recognition gap — self-consistency voting (majority/union) + blind-fallback | voting did **NOT** beat blind (best vs-blind +1.3 to +2.0 = within noise). Op-set sizes reveal why: **1.7B majority-names ~20-25 of ~22-30 ops** (over-includes → no narrowing); 360M under-names (~3-4 → misses) | **diagnosis:** the model HAS the breadth but can't **SELECT** which concepts are *relevant* to a task — relevance-discrimination is the gap, not knowledge. → **v25** force selection (ranked top-K) |
| **v25** | can the model SELECT? forced ranked top-K + recall@K (direct test) | **recall@2 at/below chance** on 6-7-cat vocabularies (1.7B lists 2%, strings 6%; rand ~5-7%); only 1.7B numbers beats chance (49%). Accuracy never reaches oracle (precision↔depth squeeze) | **decisive: the model can't rank task-relevance** — category-naming asks it to classify into *our* invented ontology, the wrong interface. → **v26** PIVOT: LLM proposes **Python code** (in-distribution), sandbox-exec + verify |
| **v26** | LLM proposes **Python code** (native channel) — sandbox-exec + verify | cut mid-run on a pivot; partial: 360M code-propose **1-2%** (can't write valid programs for these compositions) | the model's native channel is weak at small scale too; superseded by the "better LLM?" question → **v27** |
| **v27** | can the scaffold **TEACH** a better standalone LLM? (in-context expert-iteration proxy) | in-context demos **don't help, they HURT** (1.7B numbers 16.7→3.3 as M 0→8; **M=0 best across the board**) | **NEGATIVE:** verified examples of *other* tasks don't transfer to a new task's rule; the wall is the model's **intrinsic induction/generalization**. v22-27 exhaust the data/scaffold levers → next lever = **ARCHITECTURE** (trained subconscious/incubation layer) |
